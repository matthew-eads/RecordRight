...
def patient_data(id, search_id):
	...
	if visit_notes is None:
		...
		try:
			sorted_notes = sorted(visit_notes.iteritems(), key=lambda datestr: time.strptime(datestr[0], "%m/%d/%Y"))
		except:
			sorted_notes = list(visit_notes.iteritems())
	return render_template('patient_data.html', patient=patient, visit_notes=visit_notes, search_id=search_id, is_cancelled=is_cancelled)
