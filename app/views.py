    ...
    if form.validate() and request.method == 'POST':
        ...
        if users:
            if users[0].password == given_password:
                ...
                flask_session['user_id'] = users[0].id
...
def index():
    ...
    return render_template('index.html', announcements=announcements, form = form, is_admin=is_admin, user_id=flask_session['user_id'])
...
def view_reminders(id):
    ...
    return render_template('view_reminders.html', patient=patient, reminders=reminders, search_id=search_id, is_admin=is_admin)
...
@app.route('/delete_announcement/<path:id>')
@login_required
def delete_announcement(id):
    session.query(Announcement).filter(Announcement.id == id).delete()
    database.session.commit()
    return redirect(url_for('index'))
...
def delete_reminder(id, reminder_id, search_id):
    ...
    if reminder.reminder_type == 2: # recurring reminder
        ...
        proc.stdin.write("crontab -l | grep -Fv \"{}\" | crontab - ".format(reminder.cron_command))
        proc.stdin.close()
        if reminder.sched_file_prefix is not None and reminder.sched_file_prefix != "":
            proc = subprocess.Popen(["rm", "{}*".format(reminder.sched_file_prefix)])
...
def handle_single_form(single_form, patient):
    ...
    if "past" in out: 
        # 'at' won't let you set a reminder for the past, which includes today
        # unless they are being ~very~ naughty, this should only occur if they
        # pick today to set the reminder, so lets just replace it with 'NOW'
        proc = subprocess.Popen(['at', "NOW"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        proc.stdin.write(command)
        proc.stdin.close()
        out = proc.stdout.read()
        date = 'NOW'
    try:
        ...
        database.session.commit()
        # schedule a automatic deletion of this reminder
        command = "{}/delete_reminder.bash {}".format(basedir, reminder.id)
        proc = subprocess.Popen(['at', date], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        proc.stdin.write(command)
        proc.stdin.close()
        ...
        flash("Internal error, reminder was not set")
        print("output of command was: {}".format(out))
        print("error was: {}".format(e))
    

def make_script(counter_name, script_name, reminder_id, end_after):
    script = """
    #!/bin/bash
    fname="{}"
    count=`cat $fname`
    ...
    if [ $count -lt 1 ]; then
      ...
      {}/delete_reminder.bash {}
      rm $fname
      rm {}
      exit 0
    fi
    echo "not dead yet... decrementing counter"
    echo $count > $fname\n""".format(counter_name, script_name, basedir, reminder_id, script_name)
...
def handle_recurrent_form(recurrent_form, common_reminder_form, patient, single_form, search_id, is_admin):
    ...
    if (f.end_after.data is None or f.end_after.data == "") and (
        f.end_on.data is None or f.end_on.data == ""):
    ...
    reminder.message = body
    reminder.patient_id = patient.id
    # add it to the db so we can get the id
    session.add(reminder)
    session.commit()
    reminder = session.query(Reminder).filter(Reminder.id == reminder.id).first() # is this neccessary?
    ...
    if f.end_after.data is not None and f.end_after.data != '':
        ...
        counter_name = "{}/.scheduling/{}-{}.count".format(basedir, to_number, thetime)
        script_name = "{}/.scheduling/{}-{}.bash".format(basedir, to_number, thetime)
        run_script = "; bash {}".format(script_name)
        reminder.sched_file_prefix = "{}.scheduling/{}-{}".format(basedir, to_number, thetime)
        make_script(counter_name, script_name, reminder.id, f.end_after.data)
    else:
        run_script = ""
...
def create_reminder(id, search_id):
    ...
    if (recurrent_form.validate() or common_reminder_form.validate()) and request.method == 'POST':
        return handle_recurrent_form(recurrent_form=recurrent_form, common_reminder_form=common_reminder_form, 
                                     single_form=single_form, patient=patient, search_id=search_id, is_admin=is_admin)
...
def create_announcement():
    ...
    if form.validate() and request.method == 'POST':
        if form.announcement.data is not None and form.name.data is not None and form.severity.data is not None:
            today = datetime.date.today().strftime("%m/%d/%Y")
            new_announcement = Announcement(name = form.name.data, 
